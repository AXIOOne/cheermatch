...
/** Parse **bold**, *italic*, __underline__ markers into word runs per paragraph. */
...
function drawRichText(
  ...
  const lineH = size + 3;
  const pick = (w: RichRun) => (w.b ? fonts.bold : w.i ? fonts.italic : fonts.font);
  let y = yTop;
  let lines = 0;
  for (const para of parseRich(text)) {
    ...
    for (const w of para) {
      const f = pick(w);
      const spaceW = w.space && cx > 0 ? f.widthOfTextAtSize(' ', size) : 0;
      const ww = f.widthOfTextAtSize(w.text, size);
      if (cx > 0 && cx + spaceW + ww > maxW) { lines++; y -= lineH; cx = 0; }
      const sx = cx > 0 ? spaceW : 0;
      if (!measureOnly) {
        page.drawText(w.text, { x: x + cx + sx, y, size, font: f, color });
        if (w.u) page.drawLine({ start: { x: x + cx + sx, y: y - 1.5 }, end: { x: x + cx + sx + ww, y: y - 1.5 }, thickness: 0.5, color });
      }
      cx += sx + ww;
    }
    y -= lineH;
...
function formatGenerated(d = new Date()): string {
...
export async function buildScoresheetPdf(data: ScoresheetData): Promise<Uint8Array> {
  ...
  // Draws the shared scoresheet header (event title, gym/team/level, AccuScore End Time)
  // at the top of any page and returns the y-cursor below the header rule.
  const drawPageHeader = (p: PDFPage): number => {
    ...
    // event_name centered (bold serif)
    {
      ...
      p.drawText(t, {
        x: xCenter + (colW - tw) / 2,
        y: topY, size: titleFontSize, font: bold, color: TEXT,
      });
    }
    // phase right
    {
      ...
      if (t) {
        ...
        p.drawText(t, {
          x: xRight + colW - tw,
          y: topY, size: titleFontSize - 4, font: bold, color: TEXT,
        });
      }
    }

    // Sub-rows: left stack (gym, team, level/division); center hall; right accuscore time
  ...
  // ---------- Deduction Report (final page) ----------
  {
    ...
    // 5 columns: Deduction Name | Value | # Occurrences | # Warnings | Score
    ...
    for (const dr of report.rows) {
      ...
      ensureSpace(rowH + 60); // keep room for total + comments box on same page when possible
      const yBot = cursorY - rowH;
    ...
    // Total row (only the Score cell is filled, matching the sample)
    ...
    const richFonts = hasComment ? { font, bold, italic } : { font: italic, bold: italic, italic };
    const lineCount = drawRichText(page, text, 0, 0, CONTENT_W - boxPad * 2, bodySize, richFonts, bodyColor, true);
    // Ensure a minimum of 4 body lines of vertical space in the box.
